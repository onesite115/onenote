...
Feature: 
        ...
        3. ongoingTime - Ongoing Date and Time is not fully asserted at this minute. but I have asserted partially using regular experssion.
...
@ignore
@ongoing_order
Scenario: 	API - PUT /v1/orders/{orderID}/take
...
When method PUT
Then status 200
* match response contains { id:'#number',status: 'ONGOING', ongoingTime: '#regex 2018.*Z' }
* match response.status == "ONGOING"

### the below is to verify ongoing date and time, in other words to assert expected vs actual. It is still being worked. 

* def ongoingDateTime = response.ongoingTime
* print "ongoingDateTime : ", ongoingDateTime
